...
module.exports = {
	...
	async execute(interaction) {
        ...
        if (Math.random() < winPercent) {
            ...
            await interaction.reply(`<@${interaction.user.id}> ASBOLUTELY DESOLATED SOME NERD ${carName}! Congrats on their ${winnings} Corona win!`);
        } else {
            // LOSE
            newBal = userData.bal - bet;
            await interaction.reply(`<@${interaction.user.id}> lost a ${bet} Corona bet in their ${carName}, THEY GOT ABSOLUTELY REKT`);
        }
